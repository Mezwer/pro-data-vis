...
def check_champion_discrepancies(df: pd.DataFrame):
    ...
    for i, row in df.iterrows():
        ...
        if len(set(set_pickban)) != 20:
            print(
                RED
                + f"Pick/Bans do not total to 20. Found at Row {i}.{'' if SHOW_DATA else RESET}"
            )
            (
                print(f"Data is {pickban}, set check is {set_pickban}" + RESET)
                if SHOW_DATA
                else None
            )
...
def check_data_discrepancies(df: pd.DataFrame):
    ...
    for i, batch in df.groupby(df.index // 10):
        ...
        for col in cols:
            ...
            if (not IGNORE_PARTIAL or data_completeness != "partial") and (
                not IGNORE_NAN and not pd.isna(blue).all() and not pd.isna(red).all()
            ):
                ...
                if len(set(blue)) != 1:
                    print(
                        RED
                        + f"Data for blue team does not match for {col}. Found at {where}, where the data completion is: {data_completeness} and the league is {region}"
                    )
                    print(f"Data was {blue}" + RESET)
                if len(set(red)) != 1:
                    print(
                        RED
                        + f"Data for red team does not match for {col}. Found at {where}, where the data completion is: {data_completeness} and the league is {region}"
                    )
